...
namespace VinterTweaks.blockentities
{
    class BEAetherCoinsPile : BlockEntityItemPile, IBlockEntityItemPile
    {
        ...
        MeshData[] meshes
        {
            get
            {
                return ObjectCacheUtil.GetOrCreate(Api, "coinspile-meshes-" + Block.Variant["metal"], () =>
                {
                    ...
                    Shape shape = Api.Assets.TryGet("vintertweaks:shapes/block/aethercoinpile.json").ToObject<Shape>();

                    ITesselatorAPI mesher = ((ICoreClientAPI)Api).Tesselator;

                    for (int j = 0; j <= 64; j++)
                    {
                        mesher.TesselateShape(block, shape, out meshes[j], null, j * 129);
                    }
